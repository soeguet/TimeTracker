...
public class HomeController {
    ...
    public String editEmployee(@PathVariable("id") UUID employeeUuid, Model model, HttpServletRequest request) {
    ...
    public String showEmployeeWorkTimes(@PathVariable("id") UUID employeeId, Model model, HttpServletRequest request) {
        previousPageReferer(model, request);
    ...
    public String editStampTime(@PathVariable("id") UUID stampTimeId, Model model, HttpServletRequest request) {
        previousPageReferer(model, request);
    ...
    //add stamptime for one employee with id and predefined date through pathvariable
    @GetMapping("/add-stamptime-employee/{id}/{date}")
    public String addStampTime(@PathVariable("id") UUID employeeId, @PathVariable("date") String date, Model model, HttpServletRequest request) {
        previousPageReferer(model, request);
        ...
        model.addAttribute("stampTime", new AddStampTimeDTO(employeeName, date, LocalTime.now().withMinute(0).format(DateTimeFormatter.ofPattern("HH:mm"))));
        return "add/add-from-employee-overview-page";
    }
    @GetMapping("/add-stamptime-employee/{id}")
    public String addStampTimeForEmployee(@PathVariable("id") UUID employeeId, Model model, HttpServletRequest request) {
        previousPageReferer(model, request);
        String employeeName = homeService.getNameOfEmployee(employeeId);
        model.addAttribute("employeeName", employeeName);
        model.addAttribute("employees", homeService.getAllEmployees());
        model.addAttribute("stampTime", new AddStampTimeDTO(employeeName, null, LocalTime.now().withMinute(0).format(DateTimeFormatter.ofPattern("HH:mm"))));
        return "add/add-from-employee-overview-page";
    }
    ...
    public String editStampTimeOverview(@PathVariable("id") UUID stampTimeId, StampTimeDTO stampTime) {
        homeService.editStampTime(stampTimeId, stampTime);
